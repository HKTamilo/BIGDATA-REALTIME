...
Cuando `04` termine, tendrás la **vista `DatosRealTime.v_subastas_live`**. Esta vista limpia los datos
**al vuelo** (cada vez que se consulta), así que los dashboards muestran SIEMPRE lo último que llegó
por streaming. **Conectá Looker a `v_subastas_live`** (NO a `subastas_clean`, que es una foto fija):

En Looker Studio → *Create* → *Data source* → BigQuery → tu proyecto → `DatosRealTime` → **`v_subastas_live`**.
Armá **2 dashboards**:
...
> `subastas_clean` (tabla) + los `mart_*` + `proceso_log` quedan para la evidencia de limpieza y controles del informe.
