...
├── esquema_DatosTR.json ← tabla cruda TODO-STRING (a prueba de tipos)
├── funcion/  (main.py + requirements.txt)
└── sql/      (01 modelo, 02 preguntas, 03 controles)
```

> 🛡️ **A prueba de tipos:** la tabla cruda `DatosTR` ahora es **todo-STRING**, así ningún
> registro rebota a una tabla de errores aunque la API mande `precio`/`monto`/`fecreg` como texto.
> El casteo a número/fecha lo hace la transformación (`SAFE_CAST` en `subastas_clean`).
