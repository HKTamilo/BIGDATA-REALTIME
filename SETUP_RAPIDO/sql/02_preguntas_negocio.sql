-- =============================================================================
--  3 PREGUNTAS DE NEGOCIO - SUBASTAS REAL TIME  (IL 3.3 - USO/CONSUMO E INSIGHTS)
--  Equipo Big Data DUOC AVY1101-02D
--  Cada query se ejecuta sobre la capa LIMPIA y alimenta un grafico Looker.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- PREGUNTA 1: ¿Cuales son los productos (acciones) con mayor monto transado y
...
SELECT
  ...
  monto_total,
  precio_prom
FROM `DatosRealTime.mart_producto`
...
--                   vs coef_variacion.
-- Insight: nucleo del caso "subasta": que acciones fluctuan mas -> mas riesgo
...
SELECT
  ...
  coef_variacion
FROM `DatosRealTime.mart_producto`
WHERE n_transacciones >= 5          -- umbral defensivo: evita volatilidad espuria
ORDER BY rango_precio DESC
LIMIT 10;

-- -----------------------------------------------------------------------------
-- PREGUNTA 3: ¿Como se distribuye la recaudacion por forma de pago y como
--             evoluciona el flujo de transacciones en el tiempo (real time)?
...
SELECT
  ...
  ticket_promedio
FROM `DatosRealTime.mart_forma_pago`
ORDER BY monto_total DESC;

-- 3b) Pulso real time: recaudacion por minuto (ultimos 60 minutos)
SELECT
  minuto,
  SUM(n_transacciones) AS transacciones,
  SUM(monto_total)     AS monto_total
FROM `DatosRealTime.mart_serie_tiempo`
