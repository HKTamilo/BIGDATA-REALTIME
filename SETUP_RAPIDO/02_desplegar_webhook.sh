...
echo "==> Desplegando webhook '$FUNCION' como Cloud Run (Python 3.10)..."
gcloud run deploy "$FUNCION" \
  --source=./funcion \
  --function=main \
  --region="$REGION" \
...
URL="$(gcloud run services describe "$FUNCION" --region="$REGION" \
...
echo "   >>>  $URL  <<<"
echo "============================================================"
echo "(tambien quedo en SETUP_RAPIDO/URL_WEBHOOK.txt)"
