# =============================================================================
#  CLOUD FUNCTION (Cloud Run) - INGESTA REAL TIME SUBASTAS
#  Equipo Big Data DUOC AVY1101-02D
#  Recibe el JSON que la API DUOC (bdrealtimeescuelait.duoc.cl) envia por POST
#  al webhook y lo publica en el topico de Pub/Sub "registros".
#
#  Entry point:   main
#  Runtime:       Python 3.10
...
#   - Control de DUPLICIDAD      -> message_id determinista (hash) como atributo
...
TOPIC_ID = os.environ.get("PUBSUB_TOPIC", "registros")

# Campos minimos que debe traer cada registro de subasta para ser valido.
CAMPOS_REQUERIDOS = (
...
log = logging.getLogger("ingesta_subastas")
...
def _publicar(item):
    """CONTROL DE DUPLICIDAD: calcula un hash determinista del registro y lo
    envia como atributo 'msg_hash'. Permite deduplicar aguas abajo (BigQuery)
    ...
    msg_hash = hashlib.sha256(payload.encode("utf-8")).hexdigest()
    ingest_ts = datetime.now(timezone.utc).isoformat()

    future = publisher.publish(
        topic_path,
        payload.encode("utf-8"),
        msg_hash=msg_hash,          # huella del registro (dedup)
        ingest_ts=ingest_ts,        # marca de ingesta (trazabilidad)
